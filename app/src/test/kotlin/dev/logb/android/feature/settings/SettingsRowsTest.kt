...
class SettingsRowsTest {
    ...
    fun `rows carry their current value`() {
        ...
        assertEquals(listOf(SettingsPage.Appearance, SettingsPage.Account, SettingsPage.Sync, SettingsPage.Notifications, SettingsPage.Data, SettingsPage.ApiAccess, SettingsPage.About), rows.map { it.page })
        assertEquals("dark · DE", rows[0].value)
        assertEquals("ben", rows[1].value)
        assertEquals("pending:3", rows[2].value)
        assertEquals("0.1.0", rows[6].value)
    }
