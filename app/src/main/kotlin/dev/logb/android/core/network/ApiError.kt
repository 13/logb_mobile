 ...
 * if [token], the bearer the refused request actually carried (null when it carried none), is
 * still the stored one (see `SessionRepository.onUnauthorized`).
 */
class UnauthorizedException(message: String, val token: String? = null) : ApiException(401, "unauthorized", message)
