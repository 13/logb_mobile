...
class ObjectRepository(private val db: LogbDatabase, private val writer: LocalWriter, private val onWrite: () -> Unit = {}) {
    ...
    suspend fun create(d: ObjectDraft, templates: List<ReminderDraft> = emptyList(), currentReading: Long? = null, today: LocalDate = LocalDate.now()): String {
        ...
        writer.create("object", uuid) {
            db.objectDao().upsert(
                ObjectEntity(uuid, null, d.name.trim(), d.type, d.counterUnit, d.fuelUnit, d.description.trim(), d.purchaseDate, d.purchasePriceCents, null, null, d.parentUuid, now, now, null, tags = Tags.toJson(d.tags.orEmpty())),
            )
        }
    ...
    suspend fun update(uuid: String, d: ObjectDraft) {
        ...
        val changes = buildMap<String, Any?> {
            ...
            if (d.tags != null && Tags.toJson(d.tags) != o.tags) put("tags", Tags.toJson(d.tags))
        }
        if (changes.isEmpty()) return
        writer.set("object", uuid, changes) {
            db.objectDao().upsert(
                o.copy(name = d.name.trim(), type = d.type, counterUnit = d.counterUnit, fuelUnit = d.fuelUnit, description = d.description.trim(), purchaseDate = d.purchaseDate, purchasePriceCents = d.purchasePriceCents, parentUuid = d.parentUuid, tags = d.tags?.let(Tags::toJson) ?: o.tags, updatedAt = Clock.nowIso()),
            )
        }
        onWrite()
    }
