...
> Status: executed 2026-09-14. Deviations: `SortedSet.reversed()` is API 35, so `years` uses `distinct().sortedDescending()`; the two switches are whole-row toggleable (`SwitchRow`), since a tap on the label did nothing on the phone; the Statistics screen recomputes on a `COUNT || MAX(updated_at)` flow over `activities` plus `objectDao.all()`, not a version table. Verified on the phone against the seeded server: total, per-type, per-category, purchases (18 791,39 €), the tree expansion, the car's insights (0,16 €/km, 42,3 l, ≈ 122 km a month, ownership ≈ 2 502 € a year since March 2019) and the reminder estimate (≈ 28.05.2036) all equal the server's `/stats`, `/insights` and `/reminders` answers.
...
- [x] Tests: `usage estimates a counter target but never makes it due` (59 000 at 25 km/day, target 60 000, today 2026-09-13 → estimate 2026-10-11, not due); a due reminder has no estimate; `soonestDays` picks the nearer of the two.
- [x] Commit `feat: reminders estimate when the counter will reach its target`.
...
```
- [x] Insights block on the Info tab, between the fields and *Contents*, in the web's order: *Include contents* switch (only with children); ownership line (total, "≈ X a year since May 2024" or "since …"); "Not enough data yet." when no year has spend, else *Spend per month* (12 bars), *Per year*, *Per category* (category labels via `categoryLabel`), *Cost per {unit}*; fuel: *Fuel logged*, *Consumption* (per 100 unit), *Fuel cost per {unit}*; *Usage* "≈ N unit a month" (rate × 30.44 / 1000 rounded); *Usage per month* (dash for an unknown month); *Consumption per fill* with the hint.
- [x] Tests: `periodLabel("2026")=="2026"`, `periodLabel("2026-03", GERMAN)=="März"`, `monthLabel("2026-09", ENGLISH)=="Sep 26"`, `sinceLabel("2024-05-01", ENGLISH)=="May 2024"`, `fillLabel("2026-01-10", ENGLISH)=="Jan 10"`, `formatPerCounter(2_496, "EUR", GERMAN)=="0,02 €"`, `formatQuantity(41_300, "l", ENGLISH)=="41.3 l"`, `formatCentsWhole(123_456, "EUR", ENGLISH)=="€1,235"`.
- [x] Commit `feat: insights on the Info tab`.
